...
from sources import is_blocked_source
import article_body
import entity_match
import news_archive
...
** 원자력이 단순 키워드로만 등장하고 본질이 다른 주제면 무조건 noise.
   단 아래 A-2 [부서 핵심이슈] 에 실질적으로 닿으면 예외다. **
** 타국의 핵무기 프로그램은 원자력 발전 뉴스가 아니다 — 한국이 당사자가 아니면 noise. **

A-2. [부서 핵심이슈] - 원자력정책실이 실제 과제로 들고 있는 영역
아래 (1)(2)(3)을 실질적으로 다루는 기사는 원자력이 전면에 없어도 noise 로 내리지 말 것.
사업·제도·비용 쪽 소재는 원자력이라는 단어가 한 번도 안 나올 수 있다 — 그래도 부서 과제다.

(1) 10대 중점사업 - 사내 최우선 추진 사업. 단순 홍보·행사가 아니면 nice_to_know 이상.
 ① 원전건설 및 신규원전 추진 ② 원전 계속운전의 차질없는 이행
 ③ i-SMR 기술개발 및 사업화 ④ 사용후핵연료 건식저장시설 건설
 ⑤ 원전 경직성 완화를 위한 탄력운전 기술 고도화 ⑥ 이원화된 원전 수출체계 일원화
 ⑦ 한수원-웨스팅하우스 합작회사(JV) 설립 ⑧ 양수 발전소 건설
 ⑨ 새만금 태양광 사업 ⑩ 경제적·합리적 원자력 청정수소 생산기반 구축

(2) 분야별 핵심이슈
 - 원전수출(수출체계 효율화·합작법인·지분 인수) / 해외사업(수익성 개선·해외리스크·초도호기)
 - 신규원전(건설비 상승 리스크·추가 신규원전) / 계속운전 / 규제개선
 - SMR(조기 사업화 법·제도 기반, 민간시장 개방, IP 이슈)
 - 사용후핵연료(저장방식·관리방안) / 방폐물(민간기업 위탁처리·고준위 특별법·건식저장시설)
 - 12차 전기본(최적 전원믹스, 원자력과 재생·ESS) / 전원계획 법령·절차 정합성
 - RPS 일몰제·REC 현물시장 폐지 / 배출권(양수발전 구매제도 개선) / 양수발전 예비타당성평가
 - 탄력운전(양수·수소생산 등 간접적 형태) / 청정수소 생산기반 / 온배수 폐열 활용
 - 원전 안전운영(운영효율 극대화·해수온도 설계변경·해양생물 유입) / 기후변화 영향·대응
 - 갈등관리(신규원전 부지 확보·건식저장시설) / 인재양성·지식승계 / 국내외 엔지니어링 인건비

(3) 표준 주제 축 - 부서 참고자료 6개 편. 이 축에 걸리는 소재는 보고서 재료다.
 · 안전성: 내진설계·사이버보안·산업안전·유해화학물질·중대사고 대비·자연재해·후쿠시마 후속·
   방사선 비상체계·삼중수소·취수구 해양생물·해수온도 설계변경·격납건물 구조건전성
 · 전원계획: 탄소중립 국가별 원전정책·전력수요 예측·전력예비율 적정성·분산에너지 특별법·
   신재생에너지와 원자력의 관계·원전 온실가스·차세대 원전연료
 · 계속운전: 세계적 트렌드·계속운전 경제성·평가 기술기준·안전성 평가서 공개
 · 경제성: 발전원가 산정방법·신규원전 건설비 증가·연료비 상승·원자력기금 요율·안전강화비용·
   원전 보험금 규모·외부비용·원전 연료세 도입·고용 창출효과
 · 사후처리: 사용후핵연료 저장·처리·처분·사후처리충당금·원전해체기술·해체 후 부지 재이용·
   중저준위 처분장·고준위 특별법
 · 수용성: 건설사업 재개·온배수 해양생태계 영향·발전소주변지역지원사업·운영정보 공개·
   수의계약 현황·후쿠시마 오염수 방류 프로세스
 · 거시·산업: 무탄소 e벨트·산업용 전기요금·전력다소비 10대 기업 부담·제조업 해외 생산이전·
   AI·데이터센터 전력수요·GDP Top20 국가 원전 현황

** 핵심이슈에 닿아도 단순 홍보·행사 스케치·정치 공방이면 그대로 noise. **
** 핵심이슈 적합은 '버리지 말라'는 뜻이지 must_read 승격 사유가 아니다. must_read 는 A 기준 그대로. **
...
- domestic: 한국 정부·규제기관·국회 (한수원·SMR 제외). 산업부, 원안위(NSSC), KINS, 과기정통부, 국회 입법, 11·12차 전기본 등.
...
- topics (0~3개): **기사가 다루는 사건의 주제**. 본문에 낱말이 나왔다는 이유로 붙이지 말 것.
  ...
  ** 경계 규칙 (2026-09-21 실측 오분류에서 나온 것) **
  · finance 는 **원전 사업의 돈**일 때만 — 프로젝트 파이낸싱·지분 투자·PPA·자금조달·
    발전사업 수익구조. 국가 간 투자 패키지·관세 협상·산업 전반의 대미 투자는 finance 가
    아니라 security_trade 다. (오분류 예: '2000억 달러 대미 전략투자 협상' → finance ✕)
  · waste 는 사용후핵연료·방폐장이 **기사의 사건**일 때만. 협정 쟁점 목록에 이름만
    오르면 붙이지 않는다. (오분류 예: 위 협상 기사에 waste ✕)
  · security_trade 는 양자·다자 에너지 협력(MOU·협정·파트너십 포함), 수출·공급망·통상·
    제재·수출통제. **에너지 안보를 내건 협력 발표는 여기다.**
    (누락 예: '한국-IEA 아시아 에너지 안보 협력 MOU' → security_trade 가 빠졌다)
  · 같은 사건을 다룬 기사끼리 태그가 갈리면 안 된다 — 매체의 어휘가 아니라 사건을 본다.
...
- title_kr: 한국어 **뉴스 헤드라인** (30~60자). 원문을 옮긴 번역문이 아니라 헤드라인을 쓴다 — 클릭 없이 제목만으로 무슨 일이 있었는지 알 수 있어야 한다. 인명·기관명 첫 등장 시 한글(원문) 병기.
  · 형태는 `행위 주체 + 핵심 행동`. 행위 주체(기관·기업·정부명)를 원문에서 특정할 수 있으면 제목에 명시한다. (X) "정부, 신규 원전 검토" → (O) "산업통상부, 신규 원전 2기 건설 검토"
  ...
  · 원문 제목이 요점을 감추면(리드 묻기·주체 생략·클릭베이트) 원문 제목이 아니라 **기사 내용 기준**으로 새로 쓴다. 말머리(`[이슈]`·`[영상]`·`[취재수첩]`·`[○○ 칼럼]`)와 매체 브랜드는 뺀다. 단 `본문:` 이 없는 기사는 제목에 있는 것만으로 쓴다(추측 금지).
  · **본문이 새로 전하는 구체 사건(주체+행위+시점: 발동·의결·체결·발표·선포·수치)이 있으면 그것이 헤드라인이다.** 분석·전망 기사라도 그 안에 담긴 사건이 제목이 된다. "가능성 시사"·"중요성 부각"·"재편 전망" 같은 주제 서술형 제목은 본문에 구체 사건이 하나도 없을 때만 허용. (X) "미국 전력망, 인프라 투자 재편 가능성 시사" → (O) "미국, 전력망 외국산 장비 겨냥 국가비상사태 행정명령 발동"
  · **배경으로 인용된 과거 사건은 헤드라인이 될 수 없다.** `지난 6월`·`앞서`·`지난해`·`~한 바 있다`처럼 이미 지나간 결정을 되짚는 문장은 이 기사가 전하는 사건이 아니다 — 그 기사가 **지금** 말하는 일(반응·과제·점검·전망)을 주체와 함께 쓴다. 칼럼·사설·기고(opinion)는 배경 사실이 아니라 **그 글의 논지**가 제목이다. (X) "한수원, 영덕을 신규 원전 건설 예정지로 선정" (9월 칼럼이 6월 선정을 배경으로 언급) → (O) "영덕군, 신규 원전 예정지 선정 후 산불 복구·지역경제 과제 안아"
  · **사건 단계를 정확히 유지한다** — 계획·검토·추진·협의·합의·계약·승인·착공·준공은 서로 다른 사건이다. 원문이 `검토`면 `결정`으로, `추진`이면 `확정`으로, 전망이면 결정으로 올려 쓰지 말 것.
...
- event_date: 기사에 명시된 사건 발생·발표·시행·예정일을 YYYY-MM-DD로 작성. 기사 게시일을 사건일로 추정하지 말 것. 일자를 확정할 수 없으면 null.
  · **이미 일어난 날짜만 찾지 말 것 — 기사에 명시된 앞으로의 일정도 사건일이다.** 자주 해당하는 것: 인허가 심사·승인 목표일(scheduled) / 공청회·설명회 예정일(scheduled) / 입찰·의견제출 마감일(deadline) / 법령·요금 시행일(effective) / 착공·가동 목표 연도(scheduled, precision=year).
  · 과거 발생일과 미래 예정일이 둘 다 있으면 **기사의 핵심 사건 쪽**을 택한다(발표 기사면 발표된 그 예정일).
...
- 아래 4개는 0~3 정수. 0=무관/없음, 1=약함, 2=유의미, 3=강함:
  ...
  report_worthiness(부서 정책과제·보고서 재료로서의 가치 — A-2 [부서 핵심이슈] 기준)
    · 3 = 핵심이슈를 정면으로 다루고 사내 판단·대응이 필요한 새 재료
    · 2 = 핵심이슈에 닿고 수치·제도 변경·해외 선례 등 보고서에 인용할 근거가 실림
    · 1 = 핵심이슈 주변 맥락
    · 0 = 그 외 (대부분)
    · 부서 정책과제는 이런 모양이다(3점 판단 기준): 60년 이상 장기운전 대응 생애주기
      전략 / 숙련인력 세대교체 대비 지식승계 / 기후회복력 기준·투자전략 /
      AI·반도체 전력수요 대응 원전 포트폴리오 / RPS 개편에 따른 손익영향·대응전략
