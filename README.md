...
수집하고, 매일 아침 텔레그램으로 **국내/해외 투자 관점 카드 브리핑**을 보낸다.
금요일엔 주간 판세 리포트.

> **면책** — 이 저장소와 [nuclens.pages.dev](https://nuclens.pages.dev) 는 개인이
> 운영하는 **비공식 프로젝트**이며, 한국수력원자력(KHNP)의 공식 서비스·공식 입장과
> 무관합니다. 모든 콘텐츠는 공개된 뉴스의 제목·요약·출처 링크만 다룹니다.

## 라이선스·이용조건

- **코드**: 저작권 보유 (all rights reserved). 열람을 위한 공개이며, 별도 허락 없는
  재사용·재배포를 허용하지 않습니다.
- **데이터** (`archive/`, `curated.json` 등): 기사 제목·LLM 생성 요약·출처 링크로
  구성된 메타데이터이며 기사 원문을 포함하지 않습니다. 개별 기사에 대한 권리는 각
  매체에 있으며, 이 데이터의 대량 재배포·재가공을 허용하지 않습니다.
