...
def load_entity_registry(path: Path = ENTITY_REGISTRY_FILE) -> list[dict]:
    """레지스트리를 관대하게 읽는다 — 없거나 깨져도 빌드는 계속된다(발간물과
    같은 계약). 알 수 없는 type·중복 id·빈 별칭은 건너뛰고 경고만 남긴다."""
    try:
        raw = json.loads(path.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError) as error:
        print(f"[build_data] 엔티티 레지스트리 없음/손상 — 매칭 생략 ({error})")
        return []
    ...
    for row in raw.get("entities") or []:
        ...
        if not entity_id or not name_kr or not aliases:
            print(f"[build_data] 엔티티 건너뜀(필수 필드 누락): {row.get('id')!r}")
            continue
        if entity_type not in ENTITY_TYPES:
            print(f"[build_data] 엔티티 건너뜀(알 수 없는 type {entity_type!r}): {entity_id}")
            continue
        if policy not in ENTITY_MATCH_POLICIES:
            print(f"[build_data] 엔티티 건너뜀(알 수 없는 match_policy {policy!r}): {entity_id}")
            continue
        if entity_id in seen_ids:
            print(f"[build_data] 엔티티 건너뜀(중복 id): {entity_id}")
            continue
...
def _entity_alias_entries(registry: list[dict]) -> list[tuple[str, bool, dict]]:
    ...
    for order, entity in enumerate(registry):
        for alias in entity["aliases"]:
            ...
            if len(norm) < minimum:
                print(f"[build_data] 엔티티 별칭 무시(최소 길이 미달): {entity['id']}/{alias!r}")
                continue
