...
def order_issue_rows(issue_rows: list[dict],
    ...
    def within_region(row: dict) -> tuple:
        ...
        #
        # 쿨다운은 must_read 에 **적용하지 않는다**. 이 주석은 처음부터 "must_read 는
        # 며칠 이어져도 선두 자격이 있다"고 말했는데 정렬키는 등급과 무관하게
        # 눌렀다 — 쿨다운 축이 등급 축 뒤에 있을 뿐, must_read 끼리는 그 축이
        # 그대로 순위를 갈랐다.
        #
        # 그래서 09-16 에 "한·미, 미국 내 대형원전 건설 추진 및 웨스팅하우스 지분
        # 투자 협상 임박"(must_read, 그날 최고점 30.2, 기사 10건)이 09-13 에 1번
        # 이었다는 이유만으로 해외 must_read 5건 중 꼴찌 → 전체 9위로 밀렸다.
        # 협상이 '임박'으로 움직였고 기사가 5→10건으로 늘어난 날이었다.
        #
        # 60일 백테스트: 최고점 must_read 가 상위 3건 밖인 날 6일 → 4일. 1번이
        # 이틀 연속 같은 이슈가 되는 날은 2일 생기는데 둘 다 사건이 커진 날이다
        # (08-04 중국 8기 승인 1→2건, 09-11 李대통령 프랑스 6→9건). 08-17 에
        # 지적받은 "안 움직이는데 눌러앉기"와는 다르다.
        #
        # 등급이 없는 반복은 그대로 눌린다 — 그게 쿨다운의 원래 표적이었다.
        #
        ...
        # 지역 안에서만 무게를 본다. 축은 불리언까지만 — 건수 자체를 쓰면
        # 전기요금 차등 7건·다뉴브강 13건 같은 주변 주제가 올라오고(60일
        # 백테스트 44일 변동), 검증 등급을 쓰면 IAEA 보도자료(official)가
