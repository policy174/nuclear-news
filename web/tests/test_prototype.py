...
#
#   로컬 · 데이터 점검      → 켠다 (데이터 품질을 보는 자리)
#   deploy-web.yml         → 끈다 (화면 코드를 배포하는 자리)
#   crawl.yml 배포 스텝     → 끈다 (2026-08-17 추가. 여기도 배포 경로다 — 뉴스가
#                                  한산한 날 배포가 멈추면 위 실측과 같은 사고)
...
class TodayAgendaPlacementTests(unittest.TestCase):
    ...
    (2026-09-06 흐름 탭으로 옮겼다가 09-07 원복 — 히어로 제목 '이번 주 원자력,
    무엇이 달라졌나'가 주간 리포트 없이는 공약이 되기 때문. 사용자 결정.)
    """
