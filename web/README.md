...
- 원본 `embeddings.json`이 존재하면 읽기 전용으로 재사용하는 하이브리드 이슈 매칭
- 임베딩 캐시가 없을 때는 제목·태그 fallback을 사용하고 국가·시설 충돌은 병합 차단
- `public/data/issue_audit.json`에 이슈 연결 근거와 매칭 방식을 기록
...
원본 아카이브에는 쓰지 않으며, 운영 반영 전에는 계획했던 LLM 배치 백필과 사람의
표본 검수를 거쳐 교체하는 것이 좋습니다.
